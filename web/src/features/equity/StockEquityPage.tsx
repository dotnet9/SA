...
function Head({ code, name, data }: { code: string; name: string; data?: Equity }) {
  return (
    <div className="sa-pagehead">
      <div>
        ...
        <div className="sub">
          F10 公开披露
          {data?.latest ? ` · 十大股东报告期 ${data.latest.endDate}` : ''}
          {data?.latestFreeFloat ? ` · 十大流通股东报告期 ${data.latestFreeFloat.endDate}` : ''}
        </div>
      </div>
    </div>
  );
